...
`easygoogle`의 Google API 관련 기능(`GoogleDriveManager`, `GoogleSheetManager`)을 사용하려면 Google Cloud 서비스 계정 키가 필요합니다.
...
# 3. Python 코드에서 사용
python3 -c "from easygoogle import GoogleDriveManager; print('✅ 설정 완료!')"
```
...
`easygoogle`는 자동으로 모든 JSON 파일을 로드하고 할당량 초과 시 다음 계정으로 전환합니다.
